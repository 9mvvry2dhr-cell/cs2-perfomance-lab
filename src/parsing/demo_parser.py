...
class DemoParser:
  ...
  def parse(self) -> ParsedMatch:
    ...
    team_a_score = 0  # Первая команда (КТ в 1-й половине)
    team_b_score = 0  # Вторая команда (Т в 1-й половине)
    parsed_rounds = []

    # 1. Парсинг раундов и счета
    try:
      ...
      if df_events is not None and not df_events.empty:
        df_valid_rounds = df_events[
            df_events["winner"].isin([2, 3, "2", "3", "CT", "T"])
        ].copy()
        ...
        for idx, (_, row) in enumerate(df_valid_rounds.iterrows()):
          ...
          parsed_rounds.append(
              ParsedRound(
                  ...
                  winner_side=(
                      "CT" if is_ct_win else ("T" if is_t_win else "UNKNOWN")
                  ),
    ...
    if rounds_played <= 0:
      is_valid = False
      validation_error = (
          "Unable to determine rounds_played. Derived metrics skipped."
      )
